	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao bonde *${mdata.subject}*\nPor favor não seja um fdp❤️`
				...
				teks = `Tchau @${num.split('@')[0]} saiu pq n faz seqsu😂👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			switch(command) {
				...
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://photos.app.goo.gl/PCz3PNvGnAcDnb6n9`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* Shikaa\n*YOUTUBE:* Ttheu Edits\n*WPP:* wa.me/+558191153393\n*INSTA:* @matheuu__05\n\n\n*Digite *.bot* para me lhe fazer uma surpresa🧐👉👈'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'bot':
			     	memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://photos.app.goo.gl/HCHYSEz9dWp19n977`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Shika ira comer seu kool🧐🤙'})
					break
				case 'belle3':
					memein = await kagApi.memeindo()
				...
				case 'bomdia':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Bom dia, BORA ACORDA CAMBADA DE DSGC😎❤'})
					break
				case 'boatarde':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(``)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Boa tarde, Hora da seção da tarde 😎👍'})
					break
				case 'boanoite':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(``)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Slk bati mo punhet... ops😥👉👈, Boa Noite fml❤️'})
					break
				case 'belle':
					memein = await kagApi.memeindo()
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do shika:*\n\n Ttheu Edits'})
					break
				case 'mia1':
					memein = await kagApi.memeindo()
        ...
        case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse Fdp aqui @${mentioned[0].split('@')[0]} agora é admin então da uma sugada nele!!😎🤙`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse Vacilão aqui @${mentioned[0].split('@')[0]} Acabou de perder o adm, vacilo d+ vagabundo(a)🤙!`, mentioned, true)
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`KKKKKKKKKK levou ban, Respeita o shika pouha : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
